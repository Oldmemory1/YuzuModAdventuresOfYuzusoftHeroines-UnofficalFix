...
| `common/scripted_effects/yuzu_other_effects.txt` | `random_owned_pop` 在 `while` 中 + `kill_pop` / `create_pop` + `last_created_pop` 在 `while` 中（5 处） | `every_owned_pop_group` + `kill_pop_group` / `create_pop_group` + `effect` | 对照 vanilla 4.4.6：`kill_single_pop` scripted effect 使用 `kill_pop_group = { pop_group = this amount = 100 }`；ghost_signal 使用 `add_modifier` 在 `every_owned_pop_group` 上；`create_pop` 已移除，改用 `create_pop_group` + `size` 参数（1 旧 pop = 100 pop_amount） |
