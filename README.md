...
| `common/deposits/Yuzu_deposits.txt` | `job_researcher_add` / `job_head_researcher_add` / `job_culture_worker_add` 在 `planet_modifier` 中（6 个特征） | `job_physicist_add` + `job_biologist_add` + `job_engineer_add` / `job_bureaucrat_add` / `job_enforcer_add` 在 `triggered_planet_modifier` 中 + 格式塔分流 | 4.4.6 中 `researcher`/`head_researcher`/`culture_worker` 均为 fake job（`possible = always = no`），且 `job_*_add` 须在 `triggered_planet_modifier` 中配合 `exists = owner` 检查；对照 vanilla `d_migrating_forest_reserve` / `d_payback_habitat_research` |
