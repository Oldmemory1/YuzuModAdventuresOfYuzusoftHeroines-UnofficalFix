...
| `common/country_limits/.../yuzu_ship_of_size_limits.txt` | 重新定义 vanilla colossus/juggernaut 上限键（覆盖原版上限）；yuzu_star_eater `base = 100`（可造 2 艘）；ownership 列表缺 `paladin_ship_limit`/`nomadic_constructor_limit` | 删除 vanilla 键定义；仅保留 `yuzu_star_eater_ships_limit` 且 `base = 40`（= 1 × size_multiplier=40，恰 1 艘）；ownership 列表补齐 `paladin_ship_limit`/`nomadic_constructor_limit` | 对照 vanilla 4.4.6 `00_ship_of_size_limits.txt`（colossus/juggernaut `base = 100`、titan `base = 80`）；堕天神兵设计上限 1 艘；ownership 替换文件须镜像 vanilla 全部 7 项（已知限制：整体覆盖，未来 vanilla 增项需手动同步） |
