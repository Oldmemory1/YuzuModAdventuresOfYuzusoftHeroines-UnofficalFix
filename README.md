...
| `common/districts/yuzu_city_districts.txt` — 全面重构 | 4 个废弃区划（`industry`/`administrative`/`religious`/`refinery`）使用 pre-4.4.6 模式直接加岗位 + designation 切换 | 移除 4 个废弃区划，新增 3 个 `district_yuzu_city_urban_1/2/3`（对标 `district_arcology_urban_1/2/3`），新增 3 个 `slot_yuzu_arcology_urban_01/02/03` | 对照 vanilla 4.4.6 普世城：urban 区划使用 `arcology` zone set 提供双倍岗位特化（工业/铸造/工厂/科研/凝聚/堡垒/贸易）；同步更新 `yuzu_city_colony_types.txt`、`yuzu_other_effects.txt`（`save_city_district_num`/`set_yuzu_city_district`）、`Yuzu_abyss_crisis_event.txt`、localisation |
